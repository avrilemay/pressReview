...
def traduction(texte):
    ...
    # les indicateurs (opération arithmétiques, majuscules, exposants, guillemet mixte ouvert)
    math_last_c = False
    ...
    while index < texte_len:  # parcourt de la str
        ...
        # lorsqu'un espace précède un guillemet mixte ouvrant
        if c.isspace() and texte[index+1] in "'\"" and not guillemet_ouvert and index+3 < texte_len:
            math_last_c = False
            majuscule_last_c = False
            guillemet_ouvert = True  # on a déjà traité le guillemet ouvrant
            resultat += braille_symbols[c]  # on imprime l'espace
            index += 1  # on avance dans l'index
            c = texte[index]  # maj du caractère
            resultat += braille_symbols[c]  # on imprime le guillemet ouvrant
            index += 1  # on avance dans l'index
            c = texte[index]  # maj du caractère
            # si un espace suit le guillemet ouvrant avant le mot, on le supprime
            if c == " " and not texte[index+1].isspace():
                index += 1  # on avance dans l'index
                continue  # itération suivante

        # lorsqu'une str commence par un guillemet ouvrant mixte
        if index == 0 and c in "'\"" and not guillemet_ouvert and index+2 < texte_len:
            guillemet_ouvert = True  # on a déjà traité le guillemet ouvrant
            resultat += braille_symbols[c]  # on imprime le guillemet ouvrant
            index += 1  # on avance dans l'index
            c = texte[index]  # maj du caractère
            # si un espace suit le guillemet ouvrant avant le mot, on le supprime
            if c == " " and not texte[index+1].isspace():
                index += 1  # on avance dans l'index (saute l'espace)
                continue  # itération suivante

        # lorsqu'un espace précède un guillemet mixte fermant
        if c == " " and texte[index + 1] in "'\"" and guillemet_ouvert and index + 1 < texte_len:
            math_last_c = False
            majuscule_last_c = False
            guillemet_ouvert = False  # le guillemet est considéré comme fermant
            index += 1  # on saute l'espace
            c = texte[index]  # on met à jour le c
            resultat += braille_symbols[c]  # on imprime le guillemet fermant
            index += 1  # on avance dans l'index
            continue  # itération suivante

        # lorsqu'un espace précède un caractère de resserrement non mixte
        ...
        # lorsqu'un caractère de resserrement non mixte précède un espace et un caractère non blanc
        ...
        if (c == " " and not math_last_c and (index - 1) >= 0 and (texte[
            index - 1].isalpha() or texte[index - 1] in "\"'»”)]") and (index + 1) < texte_len
            and texte[index+ 1] in ",;:.?!…"):
        ...
        if c.isupper():
            ...
            if c in braille_symbols:
                ...
                resultat += c    # si c inconnu: renvoyé tel quel
        ...
        if (c == " " and index - 1 >= 0 and texte[index - 1].isdigit() and (index + 1) <
                texte_len and texte[index + 1].isdigit()):
